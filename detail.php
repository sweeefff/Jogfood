...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    if (isset($user_id)) {
        ...
        if ($rating > 0 && $review !== "") {
            $insert_review_sql = "INSERT INTO minuman_reviews (minuman_id, user_id, rating, review) VALUES ($id, $user_id, $rating, '$review')";
...
$reviews_sql = "SELECT r.rating, r.review, r.created_at, u.username FROM minuman_reviews r JOIN user u ON r.user_id = u.id WHERE r.minuman_id = $id ORDER BY r.created_at DESC";
$reviews_result = mysqli_query($conn, $reviews_sql);

// Menghitung rata-rata rating
$avg_rating_sql = "SELECT AVG(rating) AS avg_rating FROM minuman_reviews WHERE minuman_id = $id";
